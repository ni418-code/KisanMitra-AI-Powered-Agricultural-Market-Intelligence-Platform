...
export const FarmerDashboard: React.FC = () => {
  ...
  // Find the top active buyer requirement (e.g. FreshMart Tomato requirement)
  const topRequirement = buyerRequirements.find(
    (r) => r.status !== 'Completed' && r.status !== 'Accepted'
  ) || buyerRequirements[0];

  const farmerName = currentUser?.name?.split(' ')[0] || 'Ramesh';
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-24 md:pb-16">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6 space-y-6">
        {/* Top Highlight Notification Card: NEW BUYER REQUIREMENT */}
        {topRequirement && (
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
              <div className="space-y-1">
                ...
                <p className="text-xs text-amber-100 font-medium flex items-center gap-1.5">
                  ...
                  <span>{topRequirement.distanceKm || 15} km {t.distanceAway}</span>
                  <span>•</span>
                  <span>Buyer: <strong>{topRequirement.buyerName}</strong></span>
                  <span>•</span>
                  <span>Total: <strong>₹{(topRequirement.quantity * topRequirement.offerPrice).toLocaleString('en-IN')}</strong></span>
                </p>
              </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
            ...
            <Link
              to="/farmer/crops/add"
              className="bg-white rounded-3xl p-6 border-2 border-emerald-500/30 hover:border-emerald-600 shadow-card hover:shadow-float transition-all group flex flex-col justify-between"
            >
              <div>
                <div className="w-14 h-14 rounded-2xl bg-emerald-100 text-emerald-700 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
              ...
              <div className="mt-5 pt-3 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-emerald-700">
                <span>Add Crop Now</span>
                <ChevronRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </div>
            </Link>

            {/* Card 2: TODAY'S MARKET PRICE */}
            <Link
              to="/farmer/market-prices"
              className="bg-white rounded-3xl p-6 border border-slate-200/80 hover:border-emerald-400 shadow-card hover:shadow-float transition-all group flex flex-col justify-between"
            >
              <div>
                <div className="w-14 h-14 rounded-2xl bg-blue-50 text-blue-700 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                  <span className="text-3xl">📈</span>
                </div>
            ...
            {/* Card 3: BUYER REQUIREMENTS */}
            <Link
              to="/farmer/requirements"
              className="bg-white rounded-3xl p-6 border border-slate-200/80 hover:border-emerald-400 shadow-card hover:shadow-float transition-all group flex flex-col justify-between"
            >
              <div>
                <div className="w-14 h-14 rounded-2xl bg-amber-50 text-amber-700 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                  <span className="text-3xl">📋</span>
                </div>
              ...
              <div className="mt-5 pt-3 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-amber-700">
                <span>See {buyerRequirements.length} Buyer Needs</span>
                <ChevronRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </div>
            </Link>
            ...
            >
              <div>
                <div className="w-14 h-14 rounded-2xl bg-teal-50 text-teal-700 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                  <span className="text-3xl">📦</span>
                </div>
              ...
              <div className="mt-5 pt-3 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-teal-700">
                <span>Track Pickups & Payouts</span>
                <ChevronRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </div>
            </Link>
            ...
            >
              <div>
                <div className="relative w-14 h-14 rounded-2xl bg-purple-50 text-purple-700 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                  <span className="text-3xl">🔔</span>
                  {unreadFarmerNotifCount > 0 && (
                    <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white rounded-full text-[10px] font-black flex items-center justify-center">
              ...
              <div className="mt-5 pt-3 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-purple-700">
                <span>{unreadFarmerNotifCount} New Alerts</span>
                <ChevronRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </div>
            </Link>
