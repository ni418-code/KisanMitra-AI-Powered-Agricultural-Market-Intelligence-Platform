...
export const AppStateProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addBuyerRequirement = async (data: { cropId: string; quantity: number; unit: 'kg' | 'quintal'; offerPrice: number; requiredDate: string; pickupRadiusKm: number; notes?: string }) => {
    ...
    const response = await createBuyerRequirement({ cropId: crop.id, cropName: crop.name, cropImage: crop.image, ...data });
    await refresh();
  ...
  const addFarmerListing = async (data: { cropId: string; quantity: number; unit: 'kg' | 'quintal'; location: string; availableDate: string; notes?: string }) => {
    const crop = crops.find(c => c.id === data.cropId);
    if (!crop) throw new Error('Crop not found.');
    const listing = await createFarmerListing({ cropId: crop.id, cropName: crop.name, cropImage: crop.image, ...data });
    setFarmerListings(prev => [listing, ...prev]);
    return listing;
  };

  const acceptRequirement = async (requirementId: string) => {
    try {
      const response = await acceptBuyerRequirement(requirementId);
      setBuyerRequirements(prev => prev.map(r => r.id === requirementId ? response.requirement : r));
      setOrders(prev => [response.order, ...prev]);
      return response.order;
    } catch (error) { console.error('Failed to accept requirement:', error); return null; }
  };

  const declineRequirement = async (requirementId: string) => {
    try {
      const updated = await declineBuyerRequirement(requirementId);
      setBuyerRequirements(prev => prev.map(r => r.id === requirementId ? updated : r));
    } catch (error) { console.error('Failed to decline requirement:', error); }
  };

  const advanceOrderStatus = async (orderId: string, nextStatus?: OrderStatus) => {
    const order = orders.find(o => o.id === orderId);
    if (!order) return null;
    const stages: OrderStatus[] = ['posted', 'matched', 'accepted', 'pickup_scheduled', 'crop_picked_up', 'payment_completed'];
    const current = stages.indexOf(order.status);
    const target = nextStatus || stages[Math.min(current + 1, stages.length - 1)];
    try {
      const updated = await updateOrderStatus(orderId, target);
      setOrders(prev => prev.map(o => o.id === orderId ? updated : o));
      return updated;
    } catch (error) { console.error('Failed to update order status:', error); return null; }
  };

  const completeOrderPayment = async (orderId: string) => {
    try {
      const updated = await payOrder(orderId);
      setOrders(prev => prev.map(o => o.id === orderId ? updated : o));
      return updated;
    } catch (error) { console.error('Payment failed:', error); return null; }
  };

  const markNotificationAsRead = (notificationId: string) => {
    markNotificationRead(notificationId).then(updated => setNotifications(prev => prev.map(n => n.id === notificationId ? updated : n))).catch(console.error);
  };

  const markAllNotificationsAsRead = (_role: 'farmer' | 'buyer') => {
    markAllNotificationsRead().then(setNotifications).catch(console.error);
  };
